...
async function fetchDashboardData() {
    const tableBody = document.getElementById('userTableBody'); tableBody.innerHTML = '<tr><td colspan="4">Loading data...</td></tr>';
    try {
        ...
        data.users.forEach(user => { const row = document.createElement('tr'); row.innerHTML = `<td>${user.username}</td><td>${user.email}</td><td><select class="user-role-select" data-username="${user.username}"><option value="free" ${user.role === 'free' ? 'selected' : ''}>Free</option><option value="member" ${user.role === 'member' ? 'selected' : ''}>Member</option><option value="admin" ${user.role === 'admin' ? 'selected' : ''}>Admin</option></select></td><td><button class="btn-save-role" data-username="${user.username}">Save</button><button class="btn-delete" data-username="${user.username}">Delete</button></td>`; tableBody.appendChild(row); });
        addEventListenersToButtons();
    } catch (error) { console.error('Failed to fetch dashboard data:', error); if (tableBody) tableBody.innerHTML = `<tr><td colspan="4" style="color:var(--danger-color);">${error.message}</td></tr>`; }
}

async function fetchAllServersData(page = 1) {
    const tableBody = document.getElementById('serverTableBody'); tableBody.innerHTML = `<tr><td colspan="7">Loading server list on page ${page}...</td></tr>`;
    try {
        ...
        tableBody.innerHTML = ''; if (data.servers.length === 0) { tableBody.innerHTML = `<tr><td colspan="7">No servers found on page ${page}.</td></tr>`; return; }
        data.servers.forEach(server => {
            const statusClass = `status-${server.status.class}`;
            const actionButton = server.status.text === 'Suspended' ? `<button class="btn-action btn-unsuspend" data-server-id="${server.id}">Unsuspend</button>` : `<button class="btn-action btn-suspend" data-server-id="${server.id}">Suspend</button>`;
            const row = document.createElement('tr');
            row.innerHTML = `<td><div class="status"><span class="status-dot ${statusClass}"></span>${server.status.text}</div></td><td>${server.name}</td><td>${server.user_id}</td><td>${server.ram} MB</td><td>${server.disk} MB</td><td>${server.cpu}%</td><td>${actionButton}<button class="btn-delete" data-server-id="${server.id}">Delete</button></td>`;
            tableBody.appendChild(row);
        });
        addEventListenersToButtons();
    } catch (error) { console.error('Failed to fetch all servers data:', error); tableBody.innerHTML = `<tr><td colspan="7" style="color:var(--danger-color);">Error: ${error.message} Periksa API Key Anda.</td></tr>`; }
}
...
function addEventListenersToButtons() {
    document.querySelectorAll('.btn-save-role').forEach(button => { button.addEventListener('click', async (e) => { const username = e.target.dataset.username; const newRole = e.target.closest('tr').querySelector('.user-role-select').value; try { const response = await fetch('/api/admin/change-role', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, newRole }) }); const result = await response.json(); if (!response.ok) throw new Error(result.message); iziToast.success({ title: 'Success', message: result.message, position: 'topRight' }); fetchDashboardData(); } catch (error) { iziToast.error({ title: 'Error', message: error.message, position: 'topRight' }); } }); });
    document.querySelectorAll('.btn-delete[data-username]').forEach(button => { button.addEventListener('click', async (e) => { const username = e.target.dataset.username; if (confirm(`Are you sure you want to delete user "${username}"?`)) { try { const response = await fetch('/api/admin/delete-user', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username }) }); const result = await response.json(); if (!response.ok) throw new Error(result.message); iziToast.success({ title: 'Success', message: result.message, position: 'topRight' }); fetchDashboardData(); } catch (error) { iziToast.error({ title: 'Error', message: error.message, position: 'topRight' }); } } }); });
    document.querySelectorAll('.btn-delete[data-server-id]').forEach(button => { button.addEventListener('click', async (e) => { const serverId = e.target.dataset.serverId; if (confirm(`PERINGATAN: Anda akan menghapus server ID ${serverId} secara permanen dari Pterodactyl. Lanjutkan?`)) { try { const response = await fetch('/api/admin/delete-server', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ serverId }) }); const result = await response.json(); if (!response.ok) throw new Error(result.message); iziToast.success({ title: 'Success', message: result.message, position: 'topRight' }); fetchAllServersData(currentServerPage); } catch (error) { iziToast.error({ title: 'Error', message: `Gagal hapus server: ${error.message}`, position: 'topRight' }); } } }); });
    document.querySelectorAll('.btn-suspend, .btn-unsuspend').forEach(button => { button.addEventListener('click', async (e) => { const serverId = e.target.dataset.serverId; const action = e.target.classList.contains('btn-suspend') ? 'suspend' : 'unsuspend'; if (!confirm(`Are you sure you want to ${action} server ${serverId}?`)) return; try { const response = await fetch('/api/admin/server-action', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ serverId, action }) }); const result = await response.json(); if (!response.ok) throw new Error(result.message); iziToast.success({ title: 'Success', message: result.message, position: 'topRight' }); fetchAllServersData(currentServerPage); } catch (error) { iziToast.error({ title: 'Error', message: `Gagal ${action} server: ${error.message}`, position: 'topRight' }); } }); });
}